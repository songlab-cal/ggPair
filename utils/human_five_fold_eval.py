...
# Convert gene lists to uppercase for case-insensitive matching
receptor_serine_threonine_kinases_genes = [gene.upper() for gene in receptor_serine_threonine_kinases_genes]
receptor_tyrosine_kinase_genes = [gene.upper() for gene in receptor_tyrosine_kinase_genes]
receptor_tyrosine_phosphatases_genes = [gene.upper() for gene in receptor_tyrosine_phosphatases_genes]
...
def process_results(results, test_type='mismatched', imbalance_train_but_balance_test=False, model='GCNG', method='MeanProb', min_positives=4, min_negatives=4, num_seeds=20):
    aggregated_results = {}
    ...
    for seed in range(1, num_seeds + 1):
        ...
        if imbalance_train_but_balance_test == True:
            df = subsample_negatives(df)
...
def plot_metric(data, y_label, order, ax, title='dataset7: balanced Testing with real ligands'):
    distinct_colors = [
        '#FF0000',  # Red
        '#0000FF',  # Blue
        '#00FF00',  # Lime
